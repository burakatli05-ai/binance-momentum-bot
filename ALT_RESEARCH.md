...
`/latestexport` verifies the stored SHA and sends a small admin-only ZIP containing only `manifest.json`, `summary.json` and `metadata.json`; it never includes `signals.db`. Metadata identifies the snapshot, its SHA/time and the summary's source/cutoff. Missing summary is explicitly `NO_SUMMARY_AVAILABLE`; an existing summary may be older than the snapshot when no new mature outcome exists. Admin-only `/latestexportfull` adds `signals.db` in a separate full ZIP. Before upload, documents over 50,000,000 bytes are rejected with an explicit size/limit message; no upload is attempted. Package, file, HTTP/API and network failures produce an admin error message and a log entry; a failed error notification is logged. No new env or DB migration is required for this export hotfix. The [Telegram cloud Bot API documents a 50 MB upload limit](https://core.telegram.org/bots/api#senddocument); the byte ceiling is conservative. Disk access is an alternative: `latest.json`, `latest-summary.json` and daily files. No public HTTP server. Retrieve oversized full snapshots through authenticated volume/export tooling. Snapshot retention is deliberately non-destructive; provision/monitor volume capacity and define retention separately before unattended use.
